...
const updateGraphPhoto = async (userPrincipalName, photoAsBase64) => {
  const accessToken = await getGraphToken()
  const headers = { Authorization: `Bearer ${accessToken}`, ContentType: 'image/jpeg' }
  const response = await axios.put(`${GRAPH.URL}/v1.0/users/${userPrincipalName}/photo/$value`, photoAsBase64, { headers })
  return response
}
module.exports = {
