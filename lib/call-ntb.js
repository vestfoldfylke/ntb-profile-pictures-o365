...
const getNtbUsers = async () => {
  const usersCache = Cache({
    basePath: './.users-cache'
  })
  const cacheKey = 'allUsers'
  const cachedUsers = usersCache.getSync(cacheKey)
  if (cachedUsers) {
    logger('info', ['get-users', 'found valid users in cache, will use that instead of fetching new'])
    return cachedUsers
  }
  const accessToken = await getNTBtoken()
  const url = 'https://vestfold.mediebank.no/api/v1/users?limit=999'
  logger('info', ['get-users', `Running GET on URL: ${url}`])
  const { data } = await axios.get(url, { headers: { Authorization: `Bearer ${accessToken}` } })
  logger('info', ['get-users', 'got response of URL'])
  const ntbUsers = data?.users
  if (!ntbUsers) {
    throw new Error('Feilet ved henting av brukere fra NTB. Fant ingen brukere')
  }
  const mappedNtbUsers = ntbUsers.map(user => {
    return {
      email: user.email,
  ...
  logger('info', ['get-users', `got ${mappedNtbUsers.length} users from NTB`])
  usersCache.setSync(cacheKey, mappedNtbUsers, 3600)
  return mappedNtbUsers
}
module.exports = { getPhotosFromAlbum, getPhotoAsBase64, getPreviewPhotoAsBase64, updatePhoto, getPreviewPhotoRaw, getNtbUsers }